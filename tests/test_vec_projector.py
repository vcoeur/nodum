...
def test_incremental_run_embeds_nodes_with_model_id(fresh_db, fake_embedder):
    node = service.create_node(type="note", title="Photosynthesis", content="sunlight to energy")
    title_only = service.create_node(type="note", title="Empty")
...
def test_update_reembeds_and_replaces_chunks(fresh_db, fake_embedder):
    node = service.create_node(type="note", title="T", content="v1 body")
    projectors.run_projectors(names=["vec"])
    service.update_node(node.id, content="v2 body")
    projectors.run_projectors(names=["vec"])
...
def test_undone_create_drops_chunks_and_vectors(fresh_db, fake_embedder):
    keep = service.create_node(type="note", title="keep", content="xylem")
    service.create_node(type="note", title="gone", content="phloem")
    projectors.run_projectors(names=["vec"])
...
def test_rebuild_equivalence_through_undo_events(fresh_db, fake_embedder):
    ...
    service.create_node(type="note", title="B", content="transient")
...
def test_long_content_produces_overlapping_chunks(fresh_db, fake_embedder):
    ...
    node = service.create_node(type="note", title=None, content=words)
    projectors.run_projectors(names=["vec"])
    chunks, vectors = _vec_state(fresh_db)
