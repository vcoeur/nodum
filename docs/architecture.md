...
- **The MCP server holds one configured actor** (`nodum mcp serve --actor`,
  default `agent:mcp`) and loads that agent's principal with its grant set —
  every read and write the tools make is confined by the store to those
  grants (INTERIM: unauthenticated until token verification lands). The actor
