...
- **The kill switch is a third verb and is deliberately not either of the other
  ...
  provider call; the four deterministic jobs make none, so a run of those
  finishes with the stop recorded on it — and every surface says exactly that
  rather than promising a wind-down that would not arrive.
