...
- **Policies died with Q13 (§8.3 learned trust).** No policy table, no rule
  ...
  nothing else: `landing=` lets a writer file below its own grant (a ceiling,
  not a mandate), which is a choice the caller makes, not a rule the service
  applies. The graduated middle gear (queue curation) is still
...
- **The cycle is the unit of reversal, and `undo` gets out of its way.** A
  ...
  cycle is stamped with its id, `undo` **refuses a cycle-stamped event** and
  says to roll back instead, and its no-argument search skips them exactly as it
  skips events a previous undo already reversed. The alternative — teaching
...
- **A rollback is itself a cycle** (decision C5), which is how its own writes
  ...
  back while its writes are live. The reversal is an involution.
...
- **A grant is a ceiling, not a mandate** (§8.3). `Store.cap_landing` lets a
...
- **A dry run's shape follows what the dry run is for.** `consolidate
  ...
  conflicts in `conflicts` instead of raising, because a confirm dialog is
  asking a question, not attempting an operation.
