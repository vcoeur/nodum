...
| `src/views/editor/` | CodeMirror-6 Markdown source editor, slash commands (node types, Markdown scaffolds, and `/link` — the create-edge dialog anchored on the current node, which inserts the chosen target's `[[Title]]` at the caret on success), `[[` autocomplete, live Mermaid preview, autosave, the write-target picker and the landing/refusal copy (`createOutcome.ts`) |
